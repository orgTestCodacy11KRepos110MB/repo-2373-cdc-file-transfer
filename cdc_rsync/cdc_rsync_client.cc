...
absl::Status CdcRsyncClient::StartServer(const ServerArch& arch) {
  ...
  /*{
    absl::StatusOr<int> local_port = port_manager_->ReservePort(
        options_.connection_timeout_sec, arch.GetType());

    if (absl::IsResourceExhausted(local_port.status())) {
      // Port in use.
      return SetTag(local_port.status(), Tag::kAddressInUse);
    }
    if (!local_port.ok()) {
    ...
    ProcessStartInfo start_info =
        remote_util_->BuildProcessStartInfoForSshPortForward(*local_port, 44550,
                                                             /*reverse=*//*false);
  ...
  std::string server_args =
      absl::StrFormat("%i %i %s", options_.forward_port_first,
                      options_.forward_port_last, component_args);
  if (IsRemoteConnection()) {
    // Run cdc_rsync_server on the remote instance.
    std::string remote_command =
        arch.GetStartServerCommand(kExitCodeNotFound, server_args);
    assert(remote_util_);
    ...
    start_info = remote_util_->BuildProcessStartInfoForSshPortForwardAndCommand(
        44550, 44450, false, remote_command, arch.GetType());
  } else {
    ...
    start_info.command = absl::StrFormat("%s %s", server_path, server_args);
  }
  ...
  /*std::unique_ptr<Process> fwd_process;
    if (IsRemoteConnection()) {
      ...
      ProcessStartInfo start_info =
       remote_util_->BuildProcessStartInfoForSshPortForward( *local_port,
              server_listen_port_, /*reverse=*//*false);
    start_info.forward_output_to_log = true;
    fwd_process = process_factory_.Create(start_info);
    RETURN_IF_ERROR(fwd_process->Start(),
                    "Failed to start cdc_rsync_server process");
  ...
  // Connect to the socket with port |server_listen_port_|.
  ...
  assert(server_listen_port_ != 0);
  status = socket_.Connect(server_listen_port_);
  if (!status.ok()) {
...
absl::Status CdcRsyncClient::HandleServerOutput(const char* data) {
  ...
  if (server_listen_port_ == 0) {
    ...
    size_t sil_pos = server_output_.find("Server is listening");
    if (sil_pos != std::string::npos) {
      // Search backwards until we find "Port ".
      size_t port_pos = server_output_.rfind("Port ", sil_pos);
      if (port_pos == std::string::npos) {
        return MakeStatus("Failed to find 'Port' marker in server output '%s'",
                          server_output_);
      }
      assert(sil_pos > port_pos);
      server_listen_port_ =
          atoi(server_output_.substr(port_pos + 5, sil_pos - port_pos).c_str());
      if (server_listen_port_ == 0) {
